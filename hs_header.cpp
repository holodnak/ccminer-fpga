...
hs_header_read(uint8_t** data, size_t* data_len, hs_header_t* hdr) {
	if (!read_u32(data, data_len, &hdr->nonce)) //0
		return false;

	if (!read_u64(data, data_len, &hdr->time)) //4
		return false;

	if (!read_bytes(data, data_len, hdr->padding, 20)) //12
		return false;

	if (!read_bytes(data, data_len, hdr->prev_block, 32)) //32
		return false;

	if (!read_bytes(data, data_len, hdr->name_root, 32)) //64
		return false;

	if (!read_bytes(data, data_len, hdr->mask_hash, 32)) //96
		return false;

	if (!read_bytes(data, data_len, hdr->extra_nonce, 24)) //128
		return false;

	if (!read_bytes(data, data_len, hdr->reserved_root, 32)) //152
		return false;

	if (!read_bytes(data, data_len, hdr->witness_root, 32)) //184
		return false;

	if (!read_bytes(data, data_len, hdr->merkle_root, 32)) //216
		return false;

	if (!read_u32(data, data_len, &hdr->version)) //248
		return false;

	if (!read_u32(data, data_len, &hdr->bits)) //252
		return false;
...
hs_header_share_write(const hs_header_t* hdr, uint8_t** data) {
	...
	memset(&b_ctx, 0, sizeof(hs_blake2b_ctx));
	...
	*/
	...
	blake2b_ctx sph_ctx;
	blake2b_init(&sph_ctx, 32, 0, 0);
	blake2b_update(&sph_ctx, sub_header, 128);
	blake2b_final(&sph_ctx, sub_hash);

	blake2b_ctx sph_ctx2;
	blake2b_init(&sph_ctx2, 32, 0, 0);
	blake2b_update(&sph_ctx2, sub_hash, 32);
	blake2b_update(&sph_ctx2, hdr->mask_hash, 32);
	blake2b_final(&sph_ctx2, commit_hash);
...
hs_header_sub_hash(const hs_header_t* hdr, uint8_t* hash) {
	...
	blake2b_ctx sph_ctx;
	blake2b_init(&sph_ctx, 32, 0, 0);
	blake2b_update(&sph_ctx, sub, size);
	blake2b_final(&sph_ctx, hash);
...
void hs_header_share_pow(uint8_t* share, uint8_t* pad32, uint8_t* hash) {
	uint8_t left[64];
	uint8_t left2[64];
	uint8_t right[32];
...
/*
	...
	*/
	blake2b_ctx sph_ctx;
	blake2b_init(&sph_ctx, 64, 0, 0);
	blake2b_update(&sph_ctx, share, 128);
	blake2b_final(&sph_ctx, left);
	/*
	printf("left hash1: ");
	printDataFPGA(left, 64);
	printf("left hash2: ");
	printDataFPGA(left2, 64);
	*/
	// Generate right.
	hs_sha3_ctx s_ctx;
	memset(&s_ctx, 0, sizeof(hs_sha3_ctx));
	...
	*/
	blake2b_ctx sph_ctx2;
	blake2b_init(&sph_ctx2, 32, 0, 0);
	blake2b_update(&sph_ctx2, left, 64);
	blake2b_update(&sph_ctx2, pad32, 32);
	blake2b_update(&sph_ctx2, right, 32);
	blake2b_final(&sph_ctx2, hash);

	//printf("the final hash: ");
